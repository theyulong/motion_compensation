...
class CableMain:
    def __init__(self, simulation_day, log_name):
        ...
        self.env = gym.make("Cable-v3") # panda_gym step渲染 render=True
        self.log_dir = f'./simulation_theta/{self.simulation_day}/log/'
        self.models_dir = f'./simulation_theta/{self.simulation_day}/model'
        # self.env = gym.make("Cable-v1")
    ...
    def cable_TD3_load(self, day, num):
        model = TD3.load(f'./simulation_theta/{day}/model/cable_v0_TD3_{num}', env=self.env)
    ...
    def cable_TD3_again(self, day, num):
        model = TD3.load('./simulation_theta/{day}/model/cable_v0_TD3_{num}', env=self.env)
        model.set_env(self.env, force_reset=True)
        # 回调函数，定期保存模型
        checkpoint_callback = CheckpointCallback(save_freq=100, save_path="./simulation_theta/1213/log/", name_prefix="rl_model")
    ...
    def cable_DDPG_mult(self):
        ...
        model = DDPG('MultiInputPolicy', 
                    self.env, buffer_size=int(5e6),tau=0.0015,learning_starts=181*10,
                    learning_rate=1e-5, batch_size=1024, gamma= 0.99, train_freq=(int(181/4),"episode"),
    ...
    def cable_DDPG_load(self,day,num):
        model = DDPG.load(f'./simulation_theta/{day}/model/cable_v1_DDPG_{num}', env=self.env)
        for i_episode in range(3):     # cycle times
            obs = self.env.reset()
            print(f"[load]-i_episode:{i_episode}")
            for i_step in range(181):         # step
    ...
    def cable_SAC_mult(self):
        ...
        model = SAC('MultiInputPolicy', 
                    self.env, buffer_size=int(1e6),tau=0.001,learning_starts=181*10,
                    learning_rate=1e-5, batch_size=1024, gamma= 0.99, train_freq=(int(181/5),"step"), 
                    replay_buffer_class=HerReplayBuffer, 
                    policy_kwargs=policy_kwargs, 
                    verbose=1, tensorboard_log=self.log_dir)
    ...
    def cable_PPO_mult(self):
        ...
        model = PPO('MultiInputPolicy', 
                    self.env, buffer_size=int(1e6),learning_rate=1e-5,n_steps=181*10,
                    batch_size=1024, gamma= 0.99,gae_lambda=0.98,ent_coef=0.01,  
                    policy_kwargs=policy_kwargs, 
                    verbose=1, tensorboard_log=self.log_dir)
...
if __name__ == "__main__":
    # tensorboard --logdir ./simulation_theta/1220/log
    # 每次训练需要更新时间：[cable_main.py]-simulation_day and [cableLengthHer.py]-self.day
    cableMain = CableMain(simulation_day='2115',log_name='TD3')
